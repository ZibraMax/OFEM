...
O.trySomethingWeird = async function () {
	this.before_load();
	await fetch("./ID_MODEL/analyze/", {
		method: "POST",
		body: JSON.stringify({
			job_name: "TEST 1",
			user_id: "admin",
			modelJson: JSON.stringify(this.original_json),
		...
		this.original_json = JSON.parse(JSON.stringify(jsondata));
		this.reset();
...
O.gui.add(O, "trySomethingWeird"); // Button
